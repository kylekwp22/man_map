...
public:
	PMemHelper()
	{
		...
		ClosePhysicalMemory(mPMemHandle);
		...
		cout << "MY PID IS : " << (int)my_pid << endl;
		PIDK = (int)my_pid;

		

		cr3 = GetDirBase(PIDK);
		cout << "dirbase :"<< cr3 << endl;
		base = GetProcessBase(PIDK);
		cout << "procBase :" << base << endl;
	...
	{
		 // Put the PID of the process you want to read

		
		return ReadVirtual(cr3, address, buff, bufSize);
	}
	...
	{
		T buff;
		ReadVirtual(cr3, address, &buff, bufSize);
		
		return buff;
	}
	template<typename T> T WPM(SIZE_T address, T buff, DWORD bufSize)
	{

		WriteVirtual(cr3, address, &buff, bufSize);

		return buff;
	}

	bool Read(uint64_t address, LPVOID buffer, DWORD size)
	{
	...
	bool Write(uint64_t address, LPVOID buffer, DWORD size)
	{
		for (int i = 0; i < mInfoCount; i++)
		{
			if (mMemInfo[i].Start <= address && address + size <= mMemInfo[i].End)
	...
	bool ReadVirtual(uint64_t dirbase, uint64_t address, LPVOID buffer, DWORD size)
	{
	...
	bool WriteVirtual(uint64_t dirbase, uint64_t address, LPVOID buffer, DWORD size)
	{
		auto paddress = TranslateLinearAddress(dirbase, address);
